...
async function reiterarEnvio() {
    ...
    await messageListener.enviarMensajes(clients, numbers.map(num => ({ number: num.split('@')[0], clientIndex: 0 })), mensaje);
}
    ...
    try {
        rl.question('¿Cuántas cuentas de WhatsApp desea utilizar? ', async (numAccounts) => {
            try {
                ...
                conversations = messageListener.escucharMensajes(clients);
                const numbers = await leerNumeros();
                ...
                await messageListener.enviarMensajes(clients, numbers.map(num => ({ number: num.split('@')[0], clientIndex: 0 })), mensaje, imageBuffer);
            } catch (error) {
                consoleManager.printError('Error en la inicialización: ' + error);
            }
        });
    } catch (error) {
        consoleManager.printError('Error en la función main: ' + error);
    }
}
                ...
                try {
                    ...
                    if (secondNumbers.length > 0) {
                        rl.question('¿Cuál es el mensaje que deseas enviar en respuesta? ', async (mensaje) => {
                            try {
                                await messageListener.enviarMensajes(clients, secondNumbers, mensaje);
                            } catch (error) {
                                consoleManager.printError('Error al enviar mensajes en respuesta: ' + error);
                            }
                        });
                    }
                } catch (error) {
                    consoleManager.printError('Error al procesar las IDs de las conversaciones: ' + error);
                }
            });
        }
    } catch (error) {
        consoleManager.printError('Error en el manejo de teclas: ' + error);
    }
});
