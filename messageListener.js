...
const filterManager = require('./filterManager');  // Importamos el nuevo módulo
            ...
            clients[i].on('message', async (msg) => {
                try {
                    if (msg.type !== 'status') {
                        ...
                        if (msg.type === 'chat') {
                            await filterManager.processChatMessage(msg, number, conversation);
                        } else if (msg.hasMedia) {
                            await filterManager.processMediaMessage(msg);
                        }
                    }
                } catch (error) {
